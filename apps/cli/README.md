...
```text
gflick [--format human|json] <COMMAND>
  status
  device list
...
gflick status
gflick device list
gflick device show 046d:unit
gflick device set 046d:unit dpi 1600
gflick device set 046d:unit polling-rate 1000 --connection wireless
gflick device set 046d:unit lighting fixed --color ff6600
gflick --format json events --count 1
gflick completions powershell > gflick.ps1
```

`gflick-probe` remains the developer diagnostic tool, including direct-HID and
